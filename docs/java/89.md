...
在学习 EnumSet 之前，请确保了解 [Java Enums](/java-programming/enums "Java enum") 。
...
`allof()`方法创建一个枚举集，其中包含指定枚举类型`大小`的所有值。 例如，
...
**注意**：在上述程序中，我们只能插入枚举类型为`大小为`的元素。 这是因为我们使用`大小`枚举创建了空的枚举集。
