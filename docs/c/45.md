...
#### 在本教程中，您将学习 C 语言中的文件处理。您将学习在 f 的帮助下使用 fprintf（），fscanf（），fread（），fwrite（），fseek（）等在 C 中处理标准 I / O。 例子。
...
使用`stdio.h`头文件中定义的`fopen()`功能执行打开文件。
...
| 模式 | 模式的含义 | 文件不存在期间 |
| --- | --- | --- |
| `r` | 开放供阅读。 | 如果文件不存在，则`fopen()`返回 NULL。 |
| `rb` | 打开以二进制模式读取。 | If the file does not exist, `fopen()` returns NULL. |
| `w` | 开放写作。 | 如果文件存在，其内容将被覆盖。
如果文件不存在，将创建它。 |
| `wb` | 打开以二进制方式写入。 | If the file exists, its contents are overwritten.
If the file does not exist, it will be created. |
| `a` | 打开进行追加。
数据添加到文件末尾。 | 如果文件不存在，将创建它。 |
| `ab` | 打开以二进制方式追加。
数据添加到文件末尾。 | If the file does not exist, it will be created. |
| `r+` | 开放供阅读和写作。 | If the file does not exist, `fopen()` returns NULL. |
| `rb+` | 打开以二进制方式读取和写入。 | If the file does not exist, `fopen()` returns NULL. |
...
If the file does not exist, it will be created. |
| `a+` | 打开以阅读和追加。 | If the file does not exist, it will be created. |
| `ab+` | 打开以阅读和追加二进制模式。 | If the file does not exist, it will be created. |
...
要写入二进制文件，需要使用`fwrite()`功能。 这些函数采用四个参数：
