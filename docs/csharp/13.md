...
2.  如果将`test-expression`评估为`true`，
    1.  while 循环内的语句被执行。
    2.  执行后，再次评估`test-expression`。
3.  如果将`test-expression`评估为`false`，则 while 循环终止。
...
*   测试表达式`i <=5`被评估。 由于 `i` 为 1 而`1 <= 5`为`true`，因此它执行 while 循环的主体。 在此，以迭代 1 将行打印在屏幕上， `i` 的值增加 1 变为 2。
*   现在，再次评估测试表达式（`i <=5`）。 同样，这次表达式也返回`true`（2 < = 5），因此该行将打印在屏幕上，并且 `i` 的值现在增加到 3。
...
<caption>While loop execution steps</caption>
| 迭代 | 我的价值 | 我< = 5 | 总和的值 |
| --- | --- | --- | --- |
| 1 | 1 | 真正 | 0+1 = 1 |
| 2 | 2 | true | 1+2 = 3 |
...
**执行**和**，而**关键字用于创建 do ... while 循环。 它类似于 while 循环，但是它们之间有很大的不同。
...
*   在体内，将计算`乘积`并将其打印在屏幕上。 `i` 的值然后增加到 2。
*   执行完循环的主体后，将评估测试表达式`i <= 10`。 总共，do ... while 循环将运行 10 次。
...
如果 while 和 do ... while 循环中的测试表达式从不评估为`false`，则循环主体将永远运行。 这样的循环称为无限循环。
