...
它实现了[队列接口](/java-programming/queue "Java Queue Interface")。
...
为了创建优先级队列，我们​​必须导入`java.util.PriorityQueue`包。 导入程序包后，可以使用以下方法在 Java 中创建优先级队列。
...
但是，我们可以在`Comparator`界面的帮助下自定义元素的顺序。 我们将在本教程的后面部分中对此进行了解。
...
在这里，我们创建了一个名为`编号`的优先级队列。 我们已将 4 和 2 插入队列。
...
然后，我们重写`compare()`方法。 现在，该方法使元素的头成为最大数量。
