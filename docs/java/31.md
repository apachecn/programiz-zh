...
在上述示例中，我们创建了`Animal`，`Dog`类的`a1`，`d1`和`c1`对象 和`Cat`。 我们已经使用`instanceof`运算符来检查这些对象`a1`，`d1`和`c1`是否也是`对象`类的对象 。 全部输出结果为`true`。

这是因为`对象`类是`java.lang`包中定义的根类。 所有其他类都是`Object`类的子类，在 Java 中形成层次结构。
