...
在此程序中，返回类型为元组，其中包含具有该类型的变量名称。 这样的主要优点是您可以使用变量名称 `msg.name` 和 `msg.coins` 而不是 `msg.0` 和`访问结果 msg.1` 。
...
现在，方法调用是可表达的。 但是，现在我们必须使用`return of + and` 中 & `和`的参数名称`来找到两个数字的和。 现在，这使得函数体不可读。 在功能体内使用 `a` 和 `b` 代替` & `和`的`。`
