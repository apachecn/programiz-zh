...
要了解有关何时将测试表达式评估为 true（非零值）和 false（0）的更多信息，请检查[关系式](/c-programming/c-operators#relational "C Relational operators")和[逻辑运算符](/c-programming/c-operators#logical "C logical operators")。
...
当用户输入-2 时，测试表达式`number<0`被评估为 true。 因此，屏幕上显示您输入的 <samp>-2</samp> 。
...
当用户输入 7 时，测试表达式`number%2==0`被评估为 false。 因此，将执行`else`主体内部的语句。
