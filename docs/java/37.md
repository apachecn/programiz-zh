...
在上面的示例中，我们创建了一个接口`Polygon`。 它包括一个常量变量`color`和一个抽象方法`getArea()`。
...
请注意，`Rectangle`类（实现了`Polygon`接口）具有实现的方法`getArea()`。
...
*   接口还用于在 Java 中实现多重继承。 如果子类是从两个或多个类继承的，则它是多重继承。
    ...
    在此，`Rectangle`必须提供`Line`和`Polygon`的所有方法的实现。
...
然后，类`Rectangle`实现`Polygon`。`Rectangle`提供了抽象方法`getArea()`的实现，并覆盖了默认方法`getSides()`。
...
在上面的示例中，接口`Polygon`扩展了`Line`接口。 现在，如果一个类实现了`Polygon`，则它应该为`Line`和`Polygon`的所有抽象类提供实现。
