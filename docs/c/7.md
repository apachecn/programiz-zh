...
在此，将运算符`++`和`--`用作前缀。 这两个运算符还可以用作`a++`和`a--`之类的后缀。 访问此页面以了解有关[增量和减量运算符用作后缀](/article/increment-decrement-operator-difference-prefix-postfix "Increment Operator as postfix")时如何工作的更多信息。
...
| &#124;&#124; | 逻辑或。 仅当任一操作数为真时为真 | 如果 c = 5 且 d = 2，则表达式`((c==5) &#124;&#124; (d>5))`等于 1。 |
...
*   `(a != b) || (c < b)`的值为 0，因为操作数`(a != b)`和`(c < b)`均为 0（否）。
