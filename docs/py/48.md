...
这是因为`for`循环使用一个迭代器，并使用`next()`函数对其进行迭代。 升高`StopIteration`时，它自动结束。 检查这里[知道在 Python](/python-programming/iterator#for-loop-working) 中实际上是如何实现 for 循环的。
...
生成器表达式的语法类似于 Python 中的[列表理解语法。 但是方括号将替换为圆括号。](/python-programming/list#list-comprehension)
