...
在 Java 中，反射使我们能够在运行时检查和操作类，接口，构造函数，方法和字段。
...
而且，`Class`提供了一种方法`getModifier()`，该方法以整数形式返回 class 的修饰符。
...
我们还可以使用`Constructor`类提供的各种方法来检查类的不同构造函数。
...
要了解`Constructor`类的更多方法，请访问[构造函数类](https://docs.oracle.com/javase/8/docs/api/java/lang/reflect/Constructor.html)
