...
#### 在本教程中，我们将通过示例学习多态性，不同类型的多态性以及如何在 Java 中实现它们。
...
假设我们需要绘制一个圆和一个正方形。 为此，我们可以创建`多边形`类，并从中继承两个子类`圆`和`正方形`。 在这种情况下，有意义的是在这两个子类中创建具有相同名称`render()`的方法，而不是创建具有不同名称的方法。
...
*   在`a1 = new Animal()`语句中， `a1` 是指`动物`类的对象。
