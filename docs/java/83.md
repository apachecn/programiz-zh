...
在上面的代码中，我们创建了一个名为`编号`的`TreeMap`，没有任何参数。 在这种情况下，`TreeMap`中的元素会自然排序（升序）。
...
在此，`getOrDefault()`方法找不到键`五`。 因此，它返回指定的默认值`5`。
...
`headMap()`方法返回指定的`键`（作为参数传递）之前的树图的所有键/值对。
...
`tailMap()`方法返回从指定的`键`（作为参数传递）开始的树形图的所有键/值对。
...
然后，我们重写`compare()`方法以按相反顺序对元素进行排序。
