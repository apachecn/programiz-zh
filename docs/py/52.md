...
**Re** 格式 **Ex** 压缩（RegEx）是定义搜索模式的字符序列。 例如，

```py
^a...s$
```

上面的代码定义了 RegEx 模式。 模式为：**以`至`到`到`** 结尾的任何五个字母字符串。
...
| `abc daat` | 1 场比赛（在`d<u>aa</u>t`） |
| `aabc daaat` | 2 场比赛（在`<u>aa</u>bc`和`d<u>aaa</u>t`） |
| `aabc daaaat` | 2 场比赛（在`<u>aa</u>bc`和`d<u>aaa</u>at`） |

让我们再尝试一个示例。 此 RegEx `[0-9]{2, 4}`匹配至少 2 位但不超过 4 位

| Expression | String | Matched? |
| --- | --- | --- |
| `[0-9]{2,4}` | `ab123csde` | 1 场比赛（在`ab<u>123</u>csde`比赛） |
...
| --- | --- | --- |
| `a&#124;b` | `cde` | No match |
| `ade` | 1 场比赛（在`<u>a</u>de`比赛） |
| `acdbea` | 3 场比赛（在`<u>a</u>cd<u>b</u>e<u>a</u>`） |
...
| `abxz` | 1 场比赛（在`a<u>bxz</u>`比赛） |
| `axz cabxz` | 2 场比赛（在`<u>axz</u>bc ca<u>bxz</u>`） |
...
| Expression | String | Matched? |
| --- | --- | --- |
| `\d` | `12abc3` | 3 场比赛（在`<u>12</u>abc<u>3</u>`） |
| `Python` | No match |
...
| Expression | String | Matched? |
| --- | --- | --- |
| `\D` | `1ab34"50` | 3 场比赛（在`1<u>ab</u>34<u>"</u>50`） |
| `1345` | No match |
...
| Expression | String | Matched? |
| --- | --- | --- |
| `\W` | `1a2%c` | 1 场比赛（在`1<u>a</u>2<u>%</u>c`） |
| `Python` | No match |
...
您可以将`计数`作为第四个参数传递给`re.sub()`方法。 如果省略，则结果为 0。这将替换所有出现的事件。
