...
在上面的示例中，所有使用的操作数都是变量。 但是，根本没有必要。 算术运算符中使用的操作数也可以是[文字](https://www.programiz.com/java-programming/variables-primitive-data-types#literals)。 例如，
...
执行此语句时，将首先评估原始值。 然后，`编号`增加。 这就是获得`5.2`作为输出的原因。
...
等式和关系运算符确定两个操作数之间的关系。 它检查操作数是否大于，小于，等于，不等于等。 根据关系，将其评估为`true`或`false`。
...
由于`2 号`大于`1 号`，因此将表达式`1 号` > `2 号`评估为`false`。

因此，将执行`else`内部的代码块，并跳过`if`内部的代码块。

如果您不理解上面的代码，请不要担心。 如果...其他文章，您将在 [Java 中详细学习它。](/java-programming/if-else-statement "Java if...else")

现在，请记住，相等和关系运算符会比较两个操作数，并被评估为`true`或`false`。
...
这里，由于变量`测试`是`String`类型的。 因此，`instanceof`运算符返回`true`。 要了解更多信息，请访问 [Java 实例。](/java-programming/instanceof "Java instanceof")
...
| `&#124;&#124;` | 条件或：`true`，如果布尔表达式中的任何一个为`true` | 将`false &#124;&#124; true`评估为`true` |
| `&&` | 条件与：如果所有布尔表达式均为`true`，则为 true | 将`false && true`评估为`false` |
...
条件运算符或三元运算符`?:`是`if-then-else`语句的简写。 条件运算符的语法为：
...
在这里，我们使用三元运算符检查年份是否为 is 年。 要了解更多信息，请访问 [Java 三元运算符](/java-programming/ternary-operator "Java ternary operator")。
