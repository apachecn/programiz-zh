...
在此示例中，声明了类型为`int`（整数）的变量`年龄`，并且它只能存储整数值。
...
在此，声明类型为`int`的变量`年龄`并将其初始化为`24`。
...
编译器根据分配给变量的值来确定变量的类型。 在上面的示例中，`值`的类型为`int`。 这等效于：
...
2.  The variable name must start with either letter, underscore or @ symbol. For example,
    ...
    <caption>Rules for naming variables in C#</caption>
    | 变量名 | 备注 |
    | --- | --- |
    | `名称` | 有效 |
    | `subject101` | Valid |
...
3.  C# 区分大小写。 这意味着`年龄`和`年龄`涉及 2 个不同的变量。
...
1.  选择一个有意义的变量名。 例如，`名称`，`年龄`，`主题`比 `n` ， `a` 和 `s 更有意义` 。
2.  使用 **camelCase** 表示法（以小写字母开头）来命名局部变量。 例如，`的学生人数`，`年龄`等。
3.  使用 **PascalCase** 或 **CamelCase** （以大写字母开头）来命名公共成员变量。 例如，`名字`，`价格`等。
...
*   **大小**：16 位
*   **默认值**：“ \ 0”
*   **范围**：U + 0000（'\ u0000'）至 U + FFFF（'\ uffff'）
...
*   **范围**：（-7.9 x 1028 至 7.9 x 1028）/（100 至 28）
...
文字是程序中显示的固定值。 他们不需要任何计算。 例如，`5`，`false`，`'w'`是直接出现在程序中的文字，无需任何计算。
...
*   整数文字用于初始化整数数据类型的变量，即`sbyte`，`short`，`int`，`long`，`byte`，`ushort`，`uint`和`ulong`。
*   如果整数文字以`L`或`l`结尾，则其类型为 long。 为了获得最佳实践，请使用`L`（而非`l`）。
...
*   如果整数文字以`0x`开头，则它表示十六进制值。 没有前缀的数字将被视为十进制值。 在 C# 中不允许使用八进制和二进制表示形式。
...
*   浮点文字用于初始化 float 和 double 数据类型的变量。
*   如果浮点文字以后缀`f`或`F`结尾，则其类型为 float。 同样，如果以`d`或`D`结尾，则其类型为 double。 如果两个后缀都不存在，则默认为**的类型为 double。**
*   这些文字以科学计数法表示时包含 e 或 E。
...
*   字符文字用于初始化 char 数据类型的变量。
*   字符文字用单引号引起来。 例如`'x'`， `'p'`等。
...
*   字符串文字是字符文字的集合。
*   它们用双引号引起来。 例如，<samp>“ Hello”</samp> ，<samp>“ Easy Programming”</samp> 等。
