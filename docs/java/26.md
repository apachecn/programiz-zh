...
在这里，我们创建了一个名为`类型为`的字符串。 在这里，我们使用`“ java programming”` 初始化了字符串。 在 Java 中，我们使用**双引号**表示字符串。
...
在上面的示例中，我们创建了 2 个名为`的字符串，表示`和`的名称`。
...
在上面的示例中，我们将`new`关键字与构造函数`String()`一起使用来创建字符串。

`String`类提供了各种其他构造函数来创建字符串。 要了解所有这些构造函数，请访问 [Java String（Java 官方文档）](https://docs.oracle.com/javase/8/docs/api/java/lang/String.html "Official Oracle String Documentation")。

现在，让我们看一下创建字符串的过程与上一个过程有何不同。

* * *

## 使用字符串文字和 new 关键字之间的区别

现在我们知道如何使用字符串文字和`new`关键字创建字符串，让我们看看它们之间的主要区别是什么。
