...
`Person`类具有两个数据成员-`专业`和`年龄`。 它还具有两个成员函数-`walk()`和`talk()`。
...
由于可以访问`Person`的数据成员，因此设置了`教师`的`专业`和`年龄`。 使用在`Person`类中定义的`display()`功能显示此数据。 此外，将调用`MathsTeacher`类中定义的`teachMaths()`函数。
