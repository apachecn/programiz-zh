...
#### 在本教程中，您将学习有关 Python 词典的所有知识； 如何创建，访问，添加，删除元素以及各种内置方法。

Python 字典是无序的项目集合。 字典的每个项目都有一个`key/value`对。
...
我们可以使用`pop()`方法删除词典中的特定项目。 此方法删除提供了`key`的项目并返回`value`。

`popitem()`方法可用于从字典中删除并返回任意的`(key, value)`项目对。 使用`clear()`方法可以一次删除所有项目。

我们还可以使用`del`关键字删除单个项目或整个词典本身。
...
| [get（key [，d]）](/python-programming/methods/dictionary/get) | 返回`键`的值。 如果`键`不存在，则返回`d`（默认为`None`）。 |
...
| [pop（key [，d]）](/python-programming/methods/dictionary/pop) | 如果没有找到`键`，则用`键`删除该项目并返回其值或`d`。 如果未提供`d`，但未找到`键`，则它将弹出`KeyError`。 |
| [popitem（）](/python-programming/methods/dictionary/popitem) | 删除并返回任意项（**键，值**）。 如果字典为空，则升高`KeyError`。 |
| [setdefault（key [，d]）](/python-programming/methods/dictionary/setdefault) | 如果`键`在词典中，则返回相应的值。 如果不是，则插入`键`，其值为`d`，然后返回`d`（默认为`None`）。 |
| [更新（[other]）](/python-programming/methods/dictionary/update) | 使用`和其他`中的键/值对更新字典，覆盖现有键。 |
...
字典理解是一种用 Python 中的可迭代对象创建新字典的简洁明了方法。

字典理解由一个表达式对（**键：值**）和大括号`{}`中的`for`语句组成。
...
对于或[语句，词典理解可以可选地包含更多](/python-programming/if-elif-else)[。](/python-programming/for-loop)
...
要了解更多词典理解，请访问 [Python 词典理解](/python-programming/dictionary-comprehension)。
...
我们可以使用关键字`in`来测试`key`是否在词典中。 请注意，成员资格测试仅适用于`keys`，而不适用于`values`。
...
诸如`all()`，`any()`，`len()`，`cmp()`，`sorted()`等内置函数通常与词典一起使用以执行不同的任务。

| 功能 | Description |
| [all（）](/python-programming/methods/built-in/all) | 如果词典的所有键均为 True（或者词典为空），则返回`True`。 |
