...
为了创建一个弱哈希表，我们必须首先导入`java.util.WeakHashMap`包。 导入程序包后，可以使用以下方法在 Java 中创建弱哈希表。
...
如我们所见，当弱哈希图的键`两个`设置为`null`并执行垃圾回收时，该键将被删除。
...
在此，当哈希图的键`和两个`被设置为`null`并执行垃圾收集时，该键不会被删除。
