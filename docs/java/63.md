...
要解决此问题，我们可以使用`ArrayList`类。`java.util`程序包中的`ArrayList`类允许我们创建可调整大小的数组。
...
为了使用`asList()`方法，我们必须首先导入`java.util.Arrays`程序包。
...
要了解有关对数组列表进行排序的更多信息，请访问 Java ArrayList sort。
...
## Java Array 到 ArrayList

我们还可以将数组转换为数组列表。 为此，我们可以使用`Arrays`类的`asList()`方法。

要使用`asList()`，我们必须首先导入`java.util.Arrays`程序包。 例如，
