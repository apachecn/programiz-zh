...
由于`数`大于`0`。 因此，测试表达式的求值结果为`true`。 因此，将执行`if`主体内部的代码。
...
在此，`number`的值小于`0`。 因此，测试表达式`编号> 0`求值为`false`。 因此，执行`if`的主体。
...
在上面的示例中，我们有一个名为`的变量`。 这里，测试表达式检查`number`是否大于`0`（`number > 0`）。
