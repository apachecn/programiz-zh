...
相反，您可以定义一个将`age`初始化为 0 的构造函数。然后，您要做的就是创建`Person`对象，该构造函数将自动初始化`age`。
...
定义了一个构造函数，该构造函数将`length`初始化为 5，将`width`初始化为 2。

我们还具有三个附加的成员函数`GetLength(), AreaCalculation() and DisplayArea()`，以从用户处获取长度，计算面积并分别显示面积。

当创建对象`A1`和`A2`时，由于构造函数的缘故，两个对象的`length`和`width`分别初始化为 5 和 2。
...
因此，将调用不带参数的构造函数，该构造函数会将`length`初始化为 5，将`width`初始化为 2。因此，对象`A1`的面积将为 10。

对于对象`A2`，在创建对象时将 2 和 1 作为参数传递。

因此，将调用具有两个参数的构造函数，该构造函数将`length`初始化为`l`（在这种情况下为 2），并将`宽度从`初始化为`b`（1 在这种情况下）。 因此，对象`A2`的面积将为 2。
