...
#### 在本文中，您将了解 Swift 编程中的变量，常量，文字及其用例。
...
如果您尝试在 Playground 中运行以上代码，则会给我们带来编译时错误（在初始化之前使用了变量），因为它不存储/包含任何值。
...
文字是直接出现在源代码中的值。 它可以是数字，字符或字符串等。例如：“ Hello，World”，12、23.0，“ C”是文字的简单示例。 文字常用于初始化变量或常量（将值分配给它们）。
...
在上面的表达式中，`siteName`是变量，`"Apple.com"`是文字。
...
在上面的程序中，有两个整数文字`0b11111111`（二进制文字）和`1231`（十进制文字）。`11111111`的十进制值为 **255** ，因此`print(binaryNumber)`语句在屏幕上输出 255。
...
字符串文字是由双引号引起来的字符序列，而字符文字是由双引号引起来的单个字符。

**示例 8：如何在 Swift 中使用字符串和字符文字？**
...
在上面的程序中，`"C"`是字符文字，`"Swift is awesome"`是字符串文字。
...
它可以具有可选的指数，由大写或小写 e 表示。 对于指数为 exp 的十进制数字，将基数乘以 10 <sup>exp</sup> ：

**示例 9：如何在 Swift 中使用十进制文字？**
...
在上面的程序中，`12.23`和`3.14e2`是浮点文字。`3.14e2`以指数表示，并且等效于 **3.14 * 10 <sup>2</sup>** 。

**十六进制：**

十六进制浮点数必须具有由大写或小写 p 表示的指数。对于具有 exp 指数的十六进制数，基数乘以 2 <sup>exp</sup> ：

**示例 10：如何在 Swift 中使用十六进制文字？**
...
在上面的程序中，`0xFp10`和`0xFp-12`是浮点文字。`0xFp10`用指数表示，等于 15 * 210（F 表示为十进制的 15）。 因此，`print(someFloat)`在屏幕上输出 15360.0。
...
快速有两个布尔文字。 它们是`true`和`false.`。

**示例 11：如何在 Swift 中使用布尔文字？**
...
1.  选择一个有意义的名称。 例如，**变量名**比**变量 n 更有意义。**
