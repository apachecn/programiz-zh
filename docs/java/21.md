...
在此，源数组和目标数组引用不同的对象（深复制）。 因此，如果一个阵列的元素改变，则另一阵列的相应元素不变。
...
`arraycopy()`方法允许您将源阵列的指定部分复制到目标阵列。 例如，
...
*   `System.arraycopy(n1, 2, n3, 1, 2)`-`从索引`2`开始的`n1`数组的 2`个元素被复制到从`的`1`开始的索引 ] n3`阵列
