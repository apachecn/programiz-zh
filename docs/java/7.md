...
*   `printf()`-Tt 提供字符串格式设置（类似于 C / C ++编程中的 [printf）。](/cpp-programming/library-function/cstdio/printf "printf in C/C++ programming")
...
在此，首先评估变量`编号`的值。 然后，将该值连接到字符串：`“ Number =”`。
...
**注意**：我们已使用`close()`方法关闭对象。 建议一旦输入，就关闭扫描仪对象。
