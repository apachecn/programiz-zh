...
*   如果操作数之一为 true，则`OR`运算符会将其评估为`true`。
*   如果操作数之一为假，则`AND`运算符会将其评估为`false`。
...
<caption>C# unary operators</caption>
...
### 示例 6：C# 中的 Post and Pre Increment 运算符
...
我们可以看到使用`++`作为前缀和后缀的效果。 当在操作数之后使用`++`时，首先评估该值，然后将其增加`1`。 因此，声明
...
<caption>C# Bitwise and Bit Shift operators</caption>
...
<caption>C# Compound Assignment Operators</caption>
