...
*   [Java 列表接口](/java-programming/list "Java List interface")
*   [Java 队列接口](/java-programming/queue "Java Queue interface")
*   [Java 双端队列接口](/java-programming/deque "Java Deque interface")
...
*   `狗`-它是第一个将`空`作为先前地址并将 `Cat` 作为下一个地址的元素
*   `猫`-它是持有`狗`作为先前地址的地址以及`牛`作为下一地址的地址的第二个元素
...
在这里，我们使用`List`接口声明了一个链表`animals1`。 链表只能访问`List`界面的方法。
