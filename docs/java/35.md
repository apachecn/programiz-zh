...
3.  从子类构造器中显式调用超类 no-arg（默认值）或参数化构造器。
...
如果需要调用超类`Animal`的重写方法`display()`，则使用`super.display()`。
...
在此示例中，我们在超类`Animal`和子类`Dog`中都定义了相同的实例字段`类型`。
...
编译器可以自动调用 no-arg 构造器。 但是，它不能调用参数化的构造器。
