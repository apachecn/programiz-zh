...
<figure>![Element Slicing in Python](img/e1ffea4f316ee85208db934ac59a2a89.png "Element Slicing")

<figcaption>String Slicing in Python</figcaption>
...
**+** 运算符在 Python 中执行此操作。 只需将两个字符串文字一起写就可以将它们串联在一起。
...
如果我们要打印类似<samp>的文字，他说：“那里是什么？”</samp> ，我们不能使用单引号或双引号。 由于文本本身包含单引号和双引号，因此将导致出现`SyntaxError`。
...
字符串对象有许多可用的方法。 我们上面提到的`format()`方法就是其中之一。 某些常用的方法是`lower()`，`upper()`，`join()`，`split()`，`find()`和`replace()`等。这是所有[内置方法的完整列表 Python](/python-programming/methods/string) 中的字符串。
