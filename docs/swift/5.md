...
**类型**的大小以位为单位进行测量，最多可以存储`2^bit`的值。 如果您不了解`bit`，请将其视为 0 或 1 的值。
...
同样，两位系统最多可以存储`2^2 = 4`个值：（`00,01,10,11`），每个组合可以表示为：
...
一个`Int8`整数可以总共存储`2^8 = 256`个值。 即它可以存储 0 到 255 之间的数字，对不对？
...
*   **默认值**：0.0
*   **大小**：32 位浮点数。
*   **范围**：`1.2 * 10^-38`至`3.4 * 10^38`（`~6`位数字）
...
*   **范围**：`2.3 * 10^-308`至`1.7 * 10^308`（`~15`位数字）
...
除此数据类型外，Swift 中还有其他高级数据类型，例如**元组**，**可选项**，**范围**，**类**，**结构**等，您将在后面的章节中学习。
